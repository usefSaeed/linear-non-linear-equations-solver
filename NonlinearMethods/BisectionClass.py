...
class Bisection:
    ...
    def plot(self):
        ...
        plt.plot([self.lower, self.lower], [-10, 10], color="red")
        plt.plot([self.upper, self.upper], [-10, 10], color="red")
    ...
    def solve(self,exp,l):
        ...
        while i < self.maxIterations:
            self.stepSolution += "\n" + "   Iteration:" + str(i + 1)
            f1 = root
            root = round_to_1(round_to_1((self.lower + self.upper), self.precision) / 2, self.precision)
            self.stepSolution += "\n" + "New Root: " + str(root)
            ...
            if error < self.EPS:
                break
            if round_to_1(self.fl * fr, self.precision) < 0:
                self.upper = root
            else:
                self.lower = root
            i += 1
